...
% --- CubeSat Tilt Angles ---
tilt_x = 120;   % X-axis tilt angle in degrees
tilt_z = 1;   % Z-axis tilt angle in degrees
tilt_y = 0; % Y-axis tilt angle (original)
...
% --- Initialize Power Generation Arrays ---
...
for k = 1:length(t)
    ...
    % Rotations around the CubeSat axes
    ...
    % Combine attitude and tilts
    R_attitude = R_roll * R_pitch * R_yaw * R_tilt;

    % Rotation around the orbital plane
...
% --- Display Results ---
disp([num2str(max(P_total))]);
disp([num2str(mean(P_total))]);
disp(R_attitude);

% Your R_attitude matrix
R = [R_attitude];
...
% Display results
disp(['Determinant: ', num2str(det_R)]);
disp(['Orthogonality error (should be close to 0): ', num2str(orthogonality)]);
